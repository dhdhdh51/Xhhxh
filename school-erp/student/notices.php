...
$pageTitle = 'Notices';
$db = getDB();
$notices = $db->query("SELECT n.*,u.name as author FROM notices n JOIN users u ON n.published_by=u.id WHERE n.target_role IN ('all','student') AND n.is_published=1 ORDER BY n.created_at DESC");
?>
...
<div class="col-md-6 col-xl-4"><div class="card h-100"><div class="card-body">
    <div class="d-flex justify-content-between mb-2"><span class="badge bg-primary-subtle text-primary"><?= ucfirst($n['target_role']) ?></span><small class="text-muted"><?= formatDate($n['created_at']) ?></small></div>
